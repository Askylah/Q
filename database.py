...
class UserManager:
    ...
    def add_custom_persona(self, username, old_key, new_key, name, avatar, tagline, system_prompt, access_code="", on_demand_file="", on_demand_files="[]", om_enabled=True, om_turn_threshold=5, deep_memory_enabled=False):
        try:
            if not new_key:
            ...
            if existing: # Update
                c.execute("""
                    ...
                    SET persona_key=?, name=?, avatar=?, tagline=?, system_prompt=?, access_code=?, on_demand_file=?, on_demand_files=?, om_enabled=?, om_turn_threshold=?, deep_memory_enabled=?
                    WHERE id=?
                """, (new_key, name, avatar, tagline, system_prompt, access_code, on_demand_file, on_demand_files_str, 1 if om_enabled else 0, om_turn_threshold, 1 if deep_memory_enabled else 0, existing[0]))
            else: # Insert
                c.execute("""
                    INSERT INTO custom_personas 
                    (username, persona_key, name, avatar, tagline, system_prompt, is_locked, access_code, on_demand_file, on_demand_files, om_enabled, om_turn_threshold, deep_memory_enabled, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (username, new_key, name, avatar, tagline, system_prompt, 1 if access_code else 0, access_code, on_demand_file, on_demand_files_str, 1 if om_enabled else 0, om_turn_threshold, 1 if deep_memory_enabled else 0, timestamp))
            conn.commit()
            conn.close()
            return True, new_key
        except Exception as e:
    ...
    def get_custom_personas(self, username):
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("SELECT persona_key, name, avatar, tagline, system_prompt, is_locked, access_code, status, on_demand_file, on_demand_files, om_enabled, om_turn_threshold, deep_memory_enabled FROM custom_personas WHERE username COLLATE NOCASE=?", (username,))
            rows = c.fetchall()
            conn.close()
